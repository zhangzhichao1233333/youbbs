...
class AdminSetting extends BaseAction
{
    public function __construct(){
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST'){
            ...
            if($action =='base'){
                ...
                if($_POST['main_nodes'] && ($this->options['main_nodes'] != $_POST['main_nodes'] ) ){
                    $businessService = new BusinessService();
                    $tableStatus = $businessService->getTableStatus();
                    $nodes_num = $tableStatus['Auto_increment'];
                ...
                if($_POST['ext_list'] && ($this->options['ext_list'] != $_POST['ext_list'] ) ){
                ...
                foreach($this->options as $k=>$v){
                    if($k != 'site_create'){
                        ...
                        if(str_replace('\\', '', $newv)!= $v){
                            SettingsDao::where('title', $k)->update(['value' => $newv]);
                            $changed += 1;
                            // 更新原数据 去掉反斜线
                ...
                if($changed){
                    ...
                    $this->JGCache->clear('site_options');
                }
