<?php /*a:1:{s:72:"/users/sftc/work/code/youbbs/application/index/view/index/topicpage.html";i:1664357393;}*/ ?>
<?php
    ...
    if(($timestamp - $cur_user['lastreplytime']) > $options['comment_post_space']){
        ...
        if($c_con_len>=$options['comment_min_len'] && $c_con_len<=$options['comment_max_len']){
            ...
            if($page<$new_taltol_page){
                $c_content = '';
                header('location: /new/topics/'.$tid.'/'.$new_taltol_page);
                exit;
            }else{
                $cur_ucode = $new_ucode;
                $formhash = formhash();
...
if($t_obj['tags']){
    ...
    if($relative_info === FALSE){
        ...
        foreach($tag_list as $tag){
            ...
            $relative_ids[] = isset($tag_obj['ids']) ? $tag_obj['ids'] : 0 ;
        }
        // set new tags
